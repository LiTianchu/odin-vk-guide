...
# Point the linker to use self installed GLFW
...
if ! odin build "./$BUILD_TARGET" $ARGS $ADDITIONAL_ARGS $OUT/$TARGET_NAME -extra-linker-flags:"-L/usr/local/lib -Wl,-rpath,/usr/local/lib" -show-system-calls; then
